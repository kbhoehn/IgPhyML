...
void Make_Move (_move_ *move, int type, t_tree *tree)
{
  ...
  */
  ...
  {
    if(tree->c_lnL < cur_lk-tree->io->mod->s_opt->min_diff_lk_codonModels)
    {
...
/*
      //run this twice to compensate for only being able to search in one direction
      Spr_Subtree(b,b->anc_node,tree); //don't consider subtrees that prune out the root
      new_pars = tree->c_pars;
...
//  printf("doing spr_subtree %d %d \n",b->num,link->num);
  if((link != b->left) && (link != b->rght)){
	...
	//      printf("case2.1\n");
	      ...
	      printf("\nnew1 %d %lf %lf",best_move,tree->spr_list[best_move]->lnL,tree->best_lnL + tree->mod->s_opt->min_diff_lk_move);
	      //exit(EXIT_FAILURE);
...
//printf("grafting subtree\n");
  Graft_Subtree(b_target,n_link,b_residual,tree);

 // printf("grafted subtree\n");
...
//	printf("problem routine\n");
  ...
  For(i,list_size){
      move = spr_list[i];
 //     if(move->n_opp_to_link->num != tree->mod->startnode && move->n_link->num != tree->mod->startnode){
...
//	  printf("pruned subtree\n");
	...
	//	  printf("\n\n");
	  ...
	  Update_P_Lk_Along_A_Path(move->path,move->depth_path+1,tree);
		  ...
		  if(recorded_l < 0.0){
				  	init_target->up_upp=0;
				  	Fast_Br_Len(init_target,tree,0);
				  	init_target->up_upp=1;
				    move->init_target_l = init_target->l;
				    recorded_l          = init_target->l;
				  }else{
				  	init_target->l      = recorded_l;
...
//	  printf("nlink1.3: %d %d | %lf\n",move->n_link->num,move->n_opp_to_link->num,tree->c_lnL);
	  move->lnL = Triple_Dist(move->n_link,tree,-1);
//	  printf("nlink2: %d %d | %lf\n",move->n_link->num,move->n_opp_to_link->num,move->lnL);
...
//	  printf("12.2\n");
	  if(tree->mod->whichrealmodel==HLP16)Update_Ancestors_Edge(tree->noeud[tree->mod->startnode],tree->noeud[tree->mod->startnode]->v[0],tree->noeud[tree->mod->startnode]->b[0],tree); //added by Ken 7/11

	//  if(tree->mod->whichrealmodel==HLP16)Node_Stats(tree->noeud[tree->mod->startnode],tree->noeud[tree->mod->startnode]->v[0],tree);
	//  printf("\nstats\n");
...
int Try_One_Spr_Move_Triple(spr *move, t_tree *tree)
{
  ...
  printf("got this far 1 %lf | %d %d\n",tree->c_lnL,move->n_link->num,move->n_opp_to_link->num);

  Record_Br_Len(NULL,tree);
  printf("got this far 2 %lf | %d %d\n",tree->c_lnL,move->n_link->num,move->n_opp_to_link->num);
  ...
  printf("got this far 3 %lf | %d %d %lf\n",tree->c_lnL,move->n_link->num,move->n_opp_to_link->num, move->init_target_l);

  Graft_Subtree(move->b_target,move->n_link,b_residual,tree);
  if(tree->mod->whichrealmodel==HLP16){
//	 Fast_Br_Len(init_target,tree,0);
	//  Fast_Br_Len(b_residual,tree,0);
	  Lk(tree);
  }
  printf("got this 1far %lf | %d %d\n",tree->c_lnL,move->n_link->num,move->n_opp_to_link->num);
  dir_v1 = dir_v2 = dir_v0 = -1;
  For(j,3)
    {
      if(move->n_link->v[j] == move->n_opp_to_link) dir_v0 = j;
