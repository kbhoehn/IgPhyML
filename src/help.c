...
void Usage()
{
    ...
    PhyML_Printf("\t\t        012345 corresponds to GTR. This option can be used for encoding any model that is a nested\n");
    PhyML_Printf("\t\t        within GTR.\n");
    PhyML_Printf("\n");
    ...
    PhyML_Printf("\t\t                            %sBlosum62%s | %sMtMam%s | %sMtArt%s | %sHIVw%s |  %sHIVb%s | %scustom (*)%s\n",
                 ...
                 LINE,FLAT,
                 LINE,FLAT);
    PhyML_Printf("\t\t  (*) : For the custom option, a file is requested that contains the symmetric part of the rate matrix, as \n");
    PhyML_Printf("\t\t        well as the equilibrium amino-acid frequencies.  The format of the file is described in the manual\n");
    PhyML_Printf("\n");
    PhyML_Printf("\t\t%s- %sCodon%s based models:%s HLP16%s | %sGY%s (default) | %sMG%s | %sYAP%s | %sPCM%s\n",FLAT,LINE,FLAT,LINE,FLAT,
                 LINE,FLAT,
                 LINE,FLAT,
                 LINE,FLAT);
...
//    PhyML_Printf("\n");
    ...
    PhyML_Printf("%s\n\t-f (or --frequencies) %sempirical%s, %smodel%s, %soptimized%s, %sfT,fC,fA,fG%s, %sfT1,fC1,fA1,fG1,fT2,fC2,fA2,fG2,fT3,fC3,fA3,fG3%s\n",
                 BOLD,LINE,BOLD,LINE,FLAT,LINE,FLAT,LINE,FLAT);
    PhyML_Printf("\t                       %sor%s %sfC1,fC2, ... ,fC64%s\n",BOLD,FLAT,LINE,FLAT);
    PhyML_Printf("\t                       %sor%s %sfA,fR,fN,fD,fC,fQ,fE,fG,fH,fI,fL,fK,fM,fF,fP,fS,fT,fW,fY,fV%s\n",BOLD,FLAT,LINE,FLAT);
    ...
    PhyML_Printf("\n");
    PhyML_Printf("\t\t%s\"fT,fC,fA,fG\"%s : only valid for nucleotide models and codon models using the F1X4 frequency model. \n",LINE,FLAT);
    ...
    PhyML_Printf("\t\t%s\"fA,fR,fN,fD,fC,fQ,fE,fG,fH,fI,fL,fK,fM,fF,fP,fS,fT,fW,fY,fV\"%s : only valid for amino acid models. In this case \n",LINE,FLAT);
    PhyML_Printf("\t\t  the numbers correspond to the frequencies of the different amino acids.\n");
    ...
    PhyML_Printf("\t\tOnly for codon models: Which frequency model to use.\n");
    ...
    PhyML_Printf("\t\tThis option sets the transition/transversion ratio. DNA sequences only (Nucleotide or Codon).\n");
    PhyML_Printf("\t\t(This option has no effect if the model does not estimate the transition/transversion ratio)\n");
    PhyML_Printf("%s\t\tIf you do not set this parameter, ts/tv will not be used.\n",BOLD);
    PhyML_Printf("\t\t%sts/tv_ratio%s > 0: Set transition/transversion ratio to the value.\n",LINE,FLAT);
    PhyML_Printf("\t\t%sts/tv_ratio%s = e: Get maximum likelihood estimate of transition/transversion ratio.\n",LINE,FLAT);
    PhyML_Printf("\t\t%sts/tv_ratio%s = 1: Fix estimate (default).\n",LINE,FLAT);
    PhyML_Printf("\n");
    ...
    PhyML_Printf("%s\n\t-w (or --omega) %smodel%s (Required if a codon model other than HLP16 is used)%s\n",BOLD,LINE,BOLD,FLAT);
   ...
   PhyML_Printf("%s\n\t--logtree %soption%s\n",BOLD,LINE,FLAT);
    ...
    PhyML_Printf("\t\t%smethod%s = EIGEN : (default) Use eigenvalue decomposition (preferable for reversible models).\n",LINE,FLAT);
    PhyML_Printf("\t\t%smethod%s = SSPADE: Pade approximation.\n",LINE,FLAT);
